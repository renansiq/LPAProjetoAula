...
class Level:
    def __init__(self, window, name, menu_option):
        ...
        pg.time.set_timer(EVENT_ENEMY, 4000)  # a cada 2000 milisegundos (2 segundos) virá um inimigo

    def run(self):
        pg.mixer.music.load(f'./asset/{self.name}.wav')
        pg.mixer.music.play(-1)
        clock = pg.time.Clock()
        while True:
            clock.tick(60)  # FPS to jogo ( a cada segundo ele executa o loop while(true) 60x
